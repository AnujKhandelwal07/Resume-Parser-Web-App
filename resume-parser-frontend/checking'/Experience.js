...
import { setExperienceData } from "../redux/slices/resumeSlice";
...
const Experience = () => {
  ...
  const experience = useSelector((state) => state.resume.experience);
  const memoizedExperience = useMemo(() => experience || [], [experience]); // only recalculate it when experience changes.

  const parsedResume = useSelector((state) => state.resume.parsedData);
  ...
  // Load experience data from sessionStorage on mount
  useEffect(() => {
    const storedExperience = sessionStorage.getItem("experienceData");
    if (storedExperience) {
      dispatch(setExperienceData(JSON.parse(storedExperience)));
    }
  }, [dispatch]);

  useEffect(() => {
    if (parsedResume?.workExperience?.length > 0 && experience.length === 0) {
      const cleanedExperience = parsedResume.workExperience.filter((exp) =>
        [
          "companyName",
          "city",
          "profession",
          ...
          "role",
          "shiftWorked",
        ].every((key) => exp[key] && exp[key].toString().trim())
      );

      dispatch(setExperienceData(cleanedExperience));
      ...
      if (cleanedExperience.length > 0) {
        sessionStorage.setItem("experienceCompleted", "true");
      } else {
        sessionStorage.setItem("experienceCompleted", "false");
      }

      window.dispatchEvent(new Event("progressUpdated"));
    }
  }, [parsedResume, experience, dispatch]);

  useEffect(() => {
    sessionStorage.setItem(
      "experienceData",
      JSON.stringify(memoizedExperience)
    );
  }, [memoizedExperience]);
  ...
  const handleSave = async () => {
    ...
    const updatedExperience =
      editIndex !== null
        ? experience.map((exp, idx) => (idx === editIndex ? formData : exp))
        : [...experience, formData];
    ...
    try {
      ...
      dispatch(setExperienceData(updatedExperience));
  ...
  const handleDelete = (index) => {
    ...
    // Update Redux & sessionStorage
    dispatch(setExperienceData(updatedExperience));
    sessionStorage.setItem("experienceData", JSON.stringify(updatedExperience));
  };
            ...
            <TableBody>
              {experience.map((exp, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{exp.shiftWorked}</TableCell>
                  <TableCell>
                    <IconButton
                      color="primary"
